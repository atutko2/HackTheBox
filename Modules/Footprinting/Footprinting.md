...
atutko@htb[/htb]$ for i in $(cat ip-addresses.txt);do shodan host $i;done
...
```
nmap -p21 -sC -sV 10.129.33.41 -A 
Starting Nmap 7.95 ( https://nmap.org ) at 2024-11-22 12:13 EST
Stats: 0:00:53 elapsed; 0 hosts completed (1 up), 1 undergoing Script Scan
NSE Timing: About 87.50% done; ETC: 12:14 (0:00:02 remaining)
...
SF-Port21-TCP:V=7.95%I=7%D=11/22%Time=6740BBC8%P=x86_64-apple-darwin21.6.0
SF:%r(GenericLines,74,"220\x20InFreight\x20FTP\x20v1\.1\r\n500\x20Invalid\
...
```
ftp 10.129.33.41
Connected to 10.129.33.41.
220 InFreight FTP v1.1
Name (10.129.33.41:noneya): anonymous
...
Using binary mode to transfer files.
ftp> ls
229 Entering Extended Passive Mode (|||60558|)
...
150 Opening BINARY mode data connection for flag.txt (39 bytes)
    39      114.02 KiB/s 
226 Transfer complete
39 bytes received in 00:00 (0.59 KiB/s)
ftp> ^D
221 Goodbye.
```
...
This literally kept disconnecting me and the the return from nmap is Sambda smbd 4

The answer Samba smbd 4.6.2
...
```
smbclient -N -L //10.129.202.5/sambashare
...
smb: \> ls
  ...
  .bashrc                             H     3771  Tue Feb 25 07:03:22 2020
		...
		5090944 blocks of size 1024. 1765992 blocks available
...
Desktop				automate.py
Documents			flag.txt
Downloads			host.gnmap
Library				host.nmap
Movies				host.xml
Music				shell.gif
Pictures			tmp-dir
Projects			tmp.py
Public				uninstall_visual_studio.sh
Useful
...
... running nmap from my local machine doesn't work... but it does on the PWNBOX. I am connected to the VPN. HTB can be quite annoying.

`sudo nmap --script nfs* 10.129.14.128 -sV -p111,2049`

I can run `showmount -e 10.129.202.5` to get the mounts
...
To get this answer I just cd to the /var/nfs directory and cat the flag file

Enumerate the NFS service and submit the contents of the flag.txt in the "nfsshare" share as the answer. 

Got the answer for this one while trying to get the other.
...
```
dig any inlanefreight.htb @10.129.47.49

; <<>> DiG 9.10.6 <<>> any inlanefreight.htb @10.129.47.49
;; global options: +cmd
;; connection timed out; no servers could be reached
noneya@Nones-MBP ~ % dig any inlanefreight.htb @10.129.47.49

; <<>> DiG 9.10.6 <<>> any inlanefreight.htb @10.129.47.49
;; global options: +cmd
;; Got answer:
;; ->>HEADER<<- opcode: QUERY, status: NOERROR, id: 65175
...
;; Query time: 59 msec
;; SERVER: 10.129.47.49#53(10.129.47.49)
;; WHEN: Sat Nov 23 13:35:58 EST 2024
;; MSG SIZE  rcvd: 409
```
...
... tried again today and just ran `dig axfr internal.inlanefreight.htb @ip-address` and it works

What is the IPv4 address of the hostname DC1? 

Same command gives me this answer

What is the FQDN of the host where the last octet ends with "x.x.x.203"? 

`dnsenum --dnsserver 10.129.11.35 --enum -p 0 -s 0 -o subdomains.txt -f /opt/useful/seclists/Discovery/DNS/fierce-hostlist.txt dev.inlanefreight.htb`
...
`nmap -sC -sV 10.129.65.101`

InFreight ESMTP v2.11

Enumerate the SMTP service even further and find the username that exists on the system. Submit it as the answer. 

`nmap -p25 10.129.179.44 -v --script smtp-open-relay`

Nothing there

`smtp-user-enum -M VRFY -U footprint.txt -t 10.129.16.233 -w 15 -v`

I had to run this on the pwn box and I also had to run it multiple times because of the spotty connectivity to the boxes. So multiple it failed to find the user because the target refused the connections. I tried metasploit with the same results.
